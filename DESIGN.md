...
- **Q-D — Critical-VLAN / fallback posture: RESOLVED.** Critical-auth is **opt-in and fail-closed by default** (`PaeConfig.critical_vlan: Option`, `None` ⇒ the port stays closed when all servers are unreachable); `authd` additionally preserves an already-authorized session through an outage without admitting it to a fallback (`pae::session::server_unreachable`). The authenticator only **assigns** the configured VLAN id; the *posture* of each fallback VLAN (guest = most restricted, auth-fail = quarantine/remediation, critical = most permissive) is **network policy on the pre-provisioned VLAN's ACLs**, not authenticator logic. No code change.
- **Q-E — SONiC target & EAPOL trap: target = SONiC `master`; trap pending lab validation.** The `enforce-sonic` v1 CONFIG_DB mapping tracks SONiC **master** and must be re-validated as master's schema (VLAN_MEMBER / ACL_TABLE / COPP_TRAP) shifts. **Whether an EAPOL (`0x888E`) CoPP/host-interface trap is installable on the target ASIC SAI profile is UNCONFIRMED — this is the #1 integration risk**, to be validated on a SONiC-VS / real switch. `authd` already **fails closed** on a port whose trap install cannot be confirmed in ASIC_DB, so an unsupported trap disables the port rather than silently failing open.
- **Q-F — Port-unauthorized enforcement primitive: RESOLVED (implemented).** Per-port **ACL default-deny** as proposed: `enforce-sonic` installs a `DOT1X_<port>` `ACL_TABLE` with a catch-all `DENY_ALL` (DROP) plus a higher-priority `ALLOW_EAPOL` (`0x888E`) rule so a closed port can still authenticate (`enforce-sonic::schema`). Acceptability on the target ASIC is part of the Q-E lab validation.
